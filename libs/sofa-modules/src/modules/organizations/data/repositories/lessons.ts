...
export class LessonRepository implements ILessonRepository {
	...
	async join (data: { id: string, join: boolean }) {
		const d = await this.client.post<{ join: boolean }, LessonFromModel>(`/${data.id}/members/join`, { join: data.join })
		return this.mapper(d)!
	}

	async manageTeachers (data: { id: string, userId: string, add: boolean }) {
		const d = await this.client.post<{ userId: string, add: boolean }, LessonFromModel>(`/${data.id}/members/teachers`, { userId: data.userId, add: data.add })
		return this.mapper(d)!
	}
}
