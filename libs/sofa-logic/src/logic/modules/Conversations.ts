...
import { CreateConversationInput, CreateMessageInput, EndConversationInput } from '../types/forms/conversations'
...
export default class Conversations extends Common {
	...
	public async GetMessages(conversationId: string, filters: QueryParams = {}) {
		return $api.conversations.conversation.getMessages(conversationId, filters).then((response) => {
	...
	public GetMessage = (conversationId: string, messageId: string) => {
		return $api.conversations.conversation.getMessage(conversationId, messageId).then((response) => {
	...
	public async CreateConversation(data: CreateConversationInput) {
		return $api.conversations.conversation.post(null, data).then((response) => {
			this.SingleConversation = response.data
			return this.SingleConversation
		})
	}
	...
	public UpdateConversation = (title: string, uuid: string) => {
		return $api.conversations.conversation.put(null, uuid, { title: title }).then((response) => {
			this.SingleConversation = response.data
			return this.SingleConversation
		})
	}
	...
	public StarMessage = (conversationId: string, messageId: string, starred: boolean) => {
	...
	public MarkMessages = (conversationId: string) => {
		return $api.conversations.conversation.markMessagesAsRead(conversationId).then((response) => {
			return response.data
		})
	}

	public CreateMessage = (conversationId: string, CreateMessageForm: CreateMessageInput) => {
		return $api.conversations.conversation.createMessage(conversationId, CreateMessageForm).then((response) => {
			this.SingleMessage = response.data
			return this.SingleMessage
		})
	}
	...
	public DeleteConversation = (id: string) => {
		return $api.conversations.conversation.delete(id).then((response) => response.data)
	}
}
