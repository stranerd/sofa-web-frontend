<template>
	<component :is="as" :class="`mdlg:!w-full ${isWrapped ? 'w-full' : 'w-[220px]'}  shadow-custom mdlg:!shadow-none flex ${isWrapped ? 'w-full' : 'flex-row'}  items-start gap-3 px-3 py-3 justify-between rounded-custom mdlg:bg-lightGray bg-white  ${customClass}`">
		<div :class="`flex mdlg:!flex-row  ${isWrapped ? ' flex-row' : ' flex-col'} gap-2 mdlg:gap-3 items-start w-full`">
			<sofa-image-loader :photoUrl="activity.image" :customClass="`mdlg:!h-[115px]  ${isWrapped ? ' h-[100px] w-[150px]' : '  h-[120px] w-full'} mdlg:!w-[200px]  rounded-tl-[16px] rounded-br-[16px] rounded-tr-[8px] rounded-bl-[8px] relative`">
			...
			<div class="grid grid-cols-10 w-full gap-2 items-center h-full relative">
				<div :class="`flex flex-col gap-2  col-span-10 h-full w-full `">
					<div class="w-full flex flex-row items-center justify-between">
						<sofa-normal-text :customClass="'!font-bold w-full text-left !line-clamp-1'">{{ activity.title
						}}</sofa-normal-text>
						<div class="flex flex-row justify-end" v-if="hasEdit"
					...
					<div class="flex items-center gap-2 flex-grow justify-between w-full">
						<a @click.stop.prevent="Logic.Common.GoToRoute(`/profile/${activity.user.id}`)"
							...
							<sofa-avatar :size="'20'" :photoUrl="activity.user.bio.photo?.link"
