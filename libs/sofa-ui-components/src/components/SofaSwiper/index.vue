<template>
  <div :class="`w-full col-span-full ${customClass} `">
    <swiper :slides-per-view="slidePerView" :space-between="spaceBetween" @swiper="onSwiper" :freeMode="freeMode"
      :modules="modules" @slideChange="onSlideChange" :class="swiperClass" :direction="direction" :enabled="enabled"
      :initial-slide="currentSlidePosition">
      <slot />
    </swiper>
    <div v-if="showPagination" class="w-full pt-3 flex flex-row items-center gap-1 justify-center">
      <span :class="`rounded w-[25px] h-[3px] ${activeSlide == index
          ? 'bg-primaryPurple'
