	...
	<div class="w-full flex flex-col h-full overflow-y-auto gap-4 text-bodyBlack placeholder:text-grayColor text-left">
		<div class="w-full flex items-center flex-wrap gap-1 md:gap-2" v-if="factory.isFillInBlanks">
			<span v-for="(item, index) in factory.fillInBlanksAnswers" :key="index" class="flex items-center gap-1">
				...
				<SofaIcon class="h-[14px] cursor-pointer" name="circle-close"
					@click="factory.fillInBlanksAnswers.splice(index, 1)" />
			</span>
		...
		<div class="w-full flex items-center flex-wrap gap-1 md:gap-2" v-else-if="factory.isDragAnswers">
			<span v-for="(item, index) in factory.dragAnswersAnswers" :key="index" class="flex items-center gap-1">
				...
				<SofaIcon class="h-[14px] cursor-pointer" name="circle-close"
					@click="factory.dragAnswersAnswers.splice(index, 1)" />
			</span>
		...
		<div v-else class="w-full flex items-start p-4 rounded-custom bg-lightGray gap-3">
			...
			<SofaTextarea textAreaStyle="bg-transparent h-[130px] w-full resize-none"
		...
		<div v-if="!factory.isFillInBlanks && !factory.isDragAnswers" class="flex flex-col gap-4">
			<template v-if="factory.isMultipleChoice">
				<div v-for="(_, index) in factory.multipleOptions" :key="index"
					...
					<SofaTextarea :rows="1" :richEditor="true" class="flex-1" textAreaStyle="p-0"
						:placeholder="`Enter answer ${index + 1}`" v-model="factory.multipleOptions[index]" />
					<SofaIcon name="remove"
						class="w-[23px] cursor-pointer hidden group-hover:inline group-focus-within:inline"
						v-if="factory.canRemoveOption" @click="factory.removeOption(index)" />
			...
			<template v-if="factory.isWriteAnswer">
				<div v-for="(_, index) in factory.writeAnswerAnswers" :key="index"
					...
					<SofaIcon :name="Logic.Study.getShape(index)"
						:class="Logic.Study.getShapeSize(Logic.Study.getShape(index))"
						class="hidden md:inline self-start" />
					<SofaTextarea :rows="index === 0 ? 3 : 1" :richEditor="true" class="flex-1" textAreaStyle="p-0"
				...
				<div v-for="(_, index) in factory.matchSet" :key="index" class="flex items-center gap-4 group">
					<div class="flex-1 flex items-center rounded-xl px-3 py-5 border-2 border-darkLightGray bg-white gap-3">
						<SofaIcon :name="Logic.Study.getShape(index)"
							...
							class="hidden md:inline self-start" />
						<SofaTextarea :rows="1" :richEditor="true" class="flex-1" textAreaStyle="p-0"
							:placeholder="`Enter word/sentence ${index + 1}`" v-model="factory.matchSet[index].q" />
					</div>
					<div class="flex-1 flex items-center rounded-xl px-3 py-5 border-2 border-darkLightGray bg-white gap-3">
						<SofaIcon :name="Logic.Study.getShape(index)"
							:class="Logic.Study.getShapeSize(Logic.Study.getShape(index))"
							class="hidden md:inline self-start" />
						<SofaTextarea :rows="1" :richEditor="true" class="flex-1" textAreaStyle="p-0"
							placeholder="Enter answer" v-model="factory.matchSet[index].a" />
					</div>
					<SofaIcon name="remove"
						class="w-[23px] cursor-pointer hidden group-hover:inline group-focus-within:inline"
						v-if="factory.canRemoveOption" @click="factory.removeOption(index)" />
			...
			<Draggable v-if="factory.isSequence" :list="factory.sequenceAnswers" class="w-full flex flex-col gap-4"
				itemKey="">
				<template #item="{ index }">
					<div
						class="w-full group flex items-center rounded-xl px-3 py-5 border-2 border-darkLightGray bg-white gap-3">
						<SofaIcon :name="Logic.Study.getShape(index)"
							...
							class="hidden md:inline self-start" />
						<SofaTextarea :rows="1" :richEditor="true" class="flex-1" textAreaStyle="p-0"
							:placeholder="`Enter word/sentence ${index + 1}`" v-model="factory.sequenceAnswers[index]" />
						<SofaIcon name="remove"
							class="w-[23px] cursor-pointer hidden group-hover:inline group-focus-within:inline"
							v-if="factory.canRemoveOption" @click="factory.removeOption(index)" />
					</div>
