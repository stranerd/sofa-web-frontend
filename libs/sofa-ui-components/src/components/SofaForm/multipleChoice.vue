    ...
    <div class="w-full flex flex-col gap-4">
      ...
      <draggable :class="`w-full grid-cols-1 gap-3 grid  ${options.length == 0 ? 'bg-gray-100 h-[140px] rounded-lg' : ''
        ...
        <template #item="{ element, index }">
          <div @click="selectedOption = element.key" :class="`col-span-1 px-3 py-3 rounded-[15px] flex flex-row gap-3 justify-between items-center bg-white border cursor-pointer ${selectedOption == element.key ? 'border-[#83AF9B]' : 'border-[#E8E8E8]'}`">
