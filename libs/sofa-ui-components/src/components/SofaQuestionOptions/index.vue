<template>
  <div class="w-full flex flex-col flex-grow mdlg:gap-0 gap-4 relative overflow-y-auto scrollbar-none">
    <div class="flex flex-col gap-6 h-full overflow-y-auto px-4">
      ...
      <div class="w-full flex flex-col gap-4">
        <a class="w-full flex items-center gap-2 bg-white" @click="toggleOpen('timeLimit')">
          <SofaIcon class="h-[18px]" name="time-limit" />
          <SofaNormalText class="!font-bold" content="Time limit" />
          <SofaNormalText content="30s" class="ml-auto" />
        ...
        <div v-if="isOpen('timeLimit')" class="w-full flex flex-wrap gap-3">
          <a v-for="option in timeOptions" :key="option.label"
            class="rounded-lg flex px-4 py-2 items-center justify-center gap-1"
            :class="!option ? 'bg-primaryPurple text-white' : 'bg-[#EFF2F5] text-deepGray'">
            <SofaNormalText class="text-center" color="text-inherit" :content="option.label" />
          </a>
        </div>
      </div>
    </div>

    <div class="bg-white rounded-b-xl w-full p-4 border-t-2 border-[#F2F5F8] flex flex-col gap-4">
