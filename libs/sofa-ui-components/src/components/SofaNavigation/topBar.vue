<template>
  <div
    :class="` flex-row items-center w-full lg:text-sm mdlg:text-[12px] text-xs  z-[8999] space-x-2 px-3 mdlg:px-4 sticky  top-0 mdlg:!bg-white  lg:!bg-white bg-backgroundGray justify-between mdlg:!shadow-custom lg:!shadow-custom ${customClass}`"
  >
    <template v-if="type == 'main'">
      <div
        class="mdlg:!hidden lg:!hidden flex flex-row items-center justify-between w-full"
      >
        <sofa-avatar
          :size="'32'"
        ...
        <div
          class="py-4 cursor-pointer flex flex-row items-center justify-center"
        >
        ...
        <div
          class="w-[30px] h-[30px] flex flex-row items-center justify-center"
      ...
      <div
        class="hidden flex-row space-x-5 items-center justify-start flex-grow mdlg:!flex lg:!flex"
      >
        <div
          class="py-4 pr-3 cursor-pointer"
        ...
        <div
          :class="`py-4 flex flex-row items-center justify-center space-x-2 cursor-pointer ${
            tabIsActive(tab.routeTag)
              ? 'border-b-[2px] border-primaryPurple'
              : ''
          }`"
          @click="Logic.Common.GoToRoute(tab.path)"
          v-for="(tab, index) in tabs"
          :key="index"
        >
          <sofa-icon
            :customClass="tab.icon_size"
            :name="tabIsActive(tab.routeTag) ? `${tab.icon}-active` : tab.icon"
          />
          <sofa-normal-text
            :customClass="'font-bold'"
            :color="
              tabIsActive(tab.routeTag) ? 'text-primaryPurple' : 'text-darkBody'
            "
          >
            {{ tab.name }}
          </sofa-normal-text>
      ...
      <div class="hidden mdlg:!flex lg:!flex flex-row items-center space-x-4">
        <sofa-button :padding="'px-4 py-1'" @click="showAddItem()"
          >Create</sofa-button
        >
        <div
          class="flex flex-col relative"
          tabindex="10"
          @blur="showNotification = false"
        >
          <div
      ...
      <div class="md:!flex hidden flex-row items-center space-x-4">
        <template v-for="(action, index) in subpageActions" :key="index">
          <template v-if="action.isIcon">
            <div
              class="flex flex-row space-x-4 border-r-[1px] border-[#E1E6EB] items-center pr-3 cursor-pointer"
            >
              <div
                class="flex flex-row space-x-2 items-center"
                v-for="(icon, index) in action.data"
                :key="index"
                @click="icon.handler()"
              >
    ...
    >
      ...
      >
        <div
          class="bg-white w-full flex flex-col lg:!px-6 md:!space-y-4 space-y-3 px-4 pb-5 md:!rounded-[16px] rounded-t-[16px] items-center justify-center"
        >
          <notification
            :close="
              () => {
