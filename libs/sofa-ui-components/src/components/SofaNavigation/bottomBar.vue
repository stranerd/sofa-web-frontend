<template>
  <div class="grid grid-cols-5 gap-3 mdlg:!hidden items-center py-2 w-full justify-center bg-backgroundGray">
    <a v-for="tab in [
      { path: '/', icon: 'home' },
    ...
    ]" :key="tab.path ?? tab.name" class="col-span-1 flex flex-col items-center justify-center" @click="() => tab.onClick?.() ?? Logic.Common.GoToRoute(tab.path)">
      ...
      <template v-else>
        <SofaIcon :name="tab.icon" :class="{ '!fill-primaryPurple': Logic.Common.tabIsActive(tab.path), 'fill-deepGray h-[30px] pb-2': true }" />
