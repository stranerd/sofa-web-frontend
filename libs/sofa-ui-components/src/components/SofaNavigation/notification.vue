<template>
  <div
    class="w-full flex flex-col space-y-1 relative overflow-y-auto h-full flex-grow"
  >
    <div
      class="w-full flex flex-row items-center justify-between sticky top-0 left-0 bg-white py-3 z-10"
    >
    ...
    <div
      ...
      v-if="AllNotifications.results.length"
    >
      <div
        :class="`w-full flex flex-row items-start justify-between ${
          item.seen ? 'opacity-80' : ''
        }`"
    ...
    <div
      v-else
      class="w-full flex flex-col space-y-2 flex-grow items-center justify-center h-full"
    >
