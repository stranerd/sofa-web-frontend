<template>
	<div class="w-full hidden mdlg:flex flex-col gap-4 h-full overflow-y-auto scrollbar-none">
		<Draggable :list="reactiveQuestions" class="w-full flex flex-col gap-4" item-key="id" group="question-list-lg">
			<template #item="{ element, index }">
				<a class="w-full p-4 group rounded-lg gap-2 flex flex-col"
					:class="selectedQuestionId == element.id ? 'bg-skyBlue' : 'bg-lightGray'" @click="selectQuestion(element)">
					<div class="flex items-center gap-2">
						<SofaNormalText class="!font-bold" :content="`${index + 1}`" />
						<span class="w-[4px] h-[4px] rounded-full bg-deepGray" />
						<SofaNormalText class="!font-bold truncate"
						...
						<div class="flex flex-row-reverse items-center ml-auto text-bodyBlack">
							<template v-for="(user, index) in users[element.id] ?? []" :key="user.id">
								...
								<SofaAvatar v-if="index === 3" bgColor="bg-darkBody !bg-opacity-80 text-lightGray" size="28"
									class="-ml-1">
					...
					<div class="w-full h-[144px] bg-cover"
						...
						<div class="h-full w-full hidden group-hover:flex gap-3 items-center justify-center">
							...
							<a v-if="quiz.status !== 'published'" @click.stop="emits('deleteQuestion', element.id)"
								class="w-[40px] h-[40px] bg-darkLightGray opacity-50 rounded-lg flex items-center justify-center">
		...
		<a class="h-[144px] flex-shrink-0 w-full rounded-xl border-2 border-darkLightGray flex items-center justify-center"
	...
	<div
		...
		<Draggable class="w-full flex flex-nowrap gap-2 items-center whitespace-nowrap" :list="reactiveQuestions"
			...
			<template #item="{ element, index }">
				<a class="w-[48px] h-[48px] flex-shrink-0 rounded-custom items-center justify-center flex relative"
					:class="selectedQuestionId === element.id ? 'bg-primaryPurple' : 'bg-lightGray'"
					@click="selectQuestion(element)">
					<SofaNormalText :color="selectedQuestionId === element.id ? 'text-white' : 'text-bodyBlack'"
						:content="`${index + 1}`" />
					<SofaNormalText v-if="users[element.id]?.length"
						...
						color="text-lightGray" :content="`${users[element.id].length}`" />
				</a>
			</template>
		</Draggable>
