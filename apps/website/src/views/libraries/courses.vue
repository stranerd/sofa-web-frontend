		...
		<div class="w-full flex flex-col gap-3 px-4 pt-3">
			<template v-if="currentCourseData.length">
				<sofa-activity-card v-for="(activity, index) in currentCourseData" :key="index" :activity="activity"
					:isWrapped="!Logic.Common.isLarge" :hasExtra="Logic.Common.isLarge" :custom-class="'mdlg:!bg-white shadow-custom cursor-pointer relative'"
					@click="openCourse(activity)">
					<template v-slot:extra>
						<div class="relative flex flex-row z-10 justify-end" :tabindex="Math.random() * 100" @blur="activity.showMore = false">
							<sofa-icon name="more-options-horizontal" customClass="h-[6px] hidden mdlg:!inline-block !cursor-pointer" @click.stop="showMoreOptionHandler(activity)" />
