<template><expanded-layout :layoutStyle="`!w-full !justify-between !flex !flex-col !h-screen !px-0 !py-0 !overflow-y-hidden ${
  ...
  <template v-if="state != 'lobby' && state != 'leaderboard' && state != 'prepare'">
    <div :class="`py-8 w-full md:!flex hidden flex-row items-center justify-center sticky top-0 left-0 bg-white ${
        ...
        <template v-else>
          <template v-if="answerState == 'pending'">
            <div class="w-full relative h-[14px] bg-primaryGreen rounded-[8px]">
              <div class="absolute top-0 left-0 h-full bg-[#E1E6EB] rounded-[8px]" :style="`width: calc( ${
  ...
  <template v-if="answerState == 'wrong' || answerState == 'correct'">
    <div class="flex flex-row items-center gap-
                <sofa-icon
                  :customClass="'h-[22px]'"
                  :name=" `${ answerState=='wrong' ? 'white-checkbox' : 'white-wrong' }`" />
    <sofa-header-text :size="'xl'" :color="'text-white'">
    ...
    <div class="flex flex-row gap-
                <sofa-button
                  ...
                  :customClass=" ` ${ answerState=='wrong' ? '!bg-primaryRed' : '!bg-primaryGreen' } !border-[2px]
      !border-white`" :hasDoubleLayer="true" :hasDarkLayer="false" v-if="
                    mode != 'game' &&
                    mode != 'tutor_test' &&
      ...
      <sofa-button :bgColor="'bg-white'" :textColor="` ${
                    answerState == 'wrong'
                      ? '!text-primaryRed'
                      : '!text-primaryGreen'
  ...
  <div :class="`w-full flex flex-row md:!hidden justify-between items-center z-50 bg-backgroundGray px-4 py-4 sticky top-0 left-0 ${
          quizIsDarkMode ? '!bg-transparent' : ''
        } ${answerState == 'wrong' ? '!bg-primaryRed' : ''} ${
          answerState == 'correct' ? '!bg-primaryGreen' : ''
        }
    ...
    <div class="flex flex-row gap-
            ...
            >
              <sofa-button
                ...
                :textColor="'text-white'"
                :customClass=" ` ${ answerState=='wrong' ? '!bg-primaryRed' : '!bg-primaryGreen' } !border-[2px]
      !border-white`" :hasDoubleLayer="true" :hasDarkLayer="false" @click="showExplanation()" v-if="
                  mode != 'game' &&
                  mode != 'tutor_test' &&
                  questions[currentQuestionIndex].explanation
                ">
  ...
  <template v-else>
    <sofa-image-loader :photoUrl="''" class="w-full h-full flex flex-col items-center">
      ...
      <template v-else>
        <div :class="`lg:!w-[50%] mdlg:!w-[70%] md:!w-[80%] h-full w-full relative flex flex-col gap-4 ${
              state == 'leaderboard' ? 'item-center justify-center' : ''
            } py-4 px-4 md:!px-0 md:!py-6`">
          <!-- lobby -->
          <div class="w-full custom-border bg-white px-4 py-4 flex flex-row gap-
              v-if=" state=='lobby'"
            >
            ...
            <div class="w-full flex flex-col gap-3 justify-center items-center">
              ...
              <div class="w-full flex flex-row items-center justify-center gap-
                    v-if=" SingleGame">
                <div class="gap-ems-center flex flex-row">
      ...
      <template v-else>
        ...
        <div class="w-full flex flex-col h-full gap-2">
          <div class="w-full flex flex-row items-center justify-between">
            <sofa-header-text :content="SingleQuiz?.title" :size="'xl'" :customClass="'text-left !line-clamp-1'" />
          ...
          <div class="w-full flex flex-row items-start gap-ex-nowrap">
      ...
      <div :class="`w-full h-auto md:!max-h-[80%] max-h-[70%]  overflow-y-auto flex flex-col gap-3 `">
        ...
        <template v-if="GameParticipants?.length && state == 'lobby'">
          <div v-for="(user, index) in GameParticipants" :key="index" :class="`w-full flex flex-col items-center justify-center py-3 px-3 custom-border border-[6px] bg-white  ${
                    userIsGameHost(user.id)
                      ? 'border-[#7DC8FA]'
                      : 'border-transparent'
                  }`">
            <sofa-normal-text :color="
                      userIsGameHost(user.id)
                        ? 'text-[#141618]'
                        : 'text-[#141618]'
        ...
        <template v-if="state == 'leaderboard'">
          <template v-if="scoreBoardParticipants">
            <div v-for="(user, index) in scoreBoardParticipants" :key="index" :class="`w-full flex flex-row items-center justify-between py-3 px-4 custom-border  bg-white    ${
      ...
      <div :class="`absolute bottom-0 left-0 flex-row md:!flex grid grid-cols-2 md:!gap-0 gap-3 ${
        ...
        <template v-if="state == 'lobby'">
          ...
          <div class="flex flex-col md:!w-auto col-span-1">
            <sofa-button :bgColor="'bg-white'" :customClass="'!bg-white !border-[2px] !border-white  md:!w-auto w-full'"
  ...
  <rate-and-review-modal v-if="showRateQuiz" :close="
      ...
      " :canClose="true" :title="'Rate this quiz'" @on-review-submitted="
        (data) => {
          rateQuiz(data);
        }
      " />
