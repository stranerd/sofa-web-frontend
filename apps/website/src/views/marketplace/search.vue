<template><dashboard-layout :middleSessionWidth="'lg:w-[78%] mdlg:w-[78%]'" :hideSmNavigator="{
      bottom: true,
      top: true,
    }">
  ...
  <template v-slot:middle-session>
    <div class="w-full flex flex-col mdlg:!gap-5 gap-3 px-0 mdlg:!pr-7 relative">
      <div
        class="w-full mdlg:!shadow-custom mdlg:!px-4 sticky mdlg:!relative top-0 px-4 left-0 mdlg:!top-auto mdlg:!left-auto z-30 mdlg:!py-1 pl-2 pr-4 py-4 pb-2 mdlg:!bg-white bg-backgroundGray mdlgcustom-border flex flex-row gap-ems-center mdlg:!justify-between justify-start">
      ...
      <div class="w-full flex flex-row gap-ems-center mdlg:px-0 pl-4">
        <span :class="`px-6 py-2  ${
              item.id == selectedFilterOption ? 'bg-primaryPurple' : 'bg-white'
            } custom-border flex flex-row items-center justify-center gap-ursor-pointer `"
      ...
      <div class="w-full flex flex-col gap-3 mdlg:!pt-0 pt-0 mdlg:px-0 pl-4" v-if="
        ...
        <template v-if="resourceContents.length">
          <div class="w-full flex flex-col gap-3">
            <div class="w-full mdlg:!grid mdlg:grid-cols-4 lg:grid-cols-5 mdlg:!gap-4" v-if="
            ...
            <div
              class="lg:!w-full mdlg:!hidden flex flex-row gap-lg:!gap-gap-owrap overflow-x-auto overflow-y-hidden scrollbar-hide">
              <div class="mdlg:!w-full mdlg:!flex mdlg:!flex-col mdlg:!gap-4 flex flex-row gap-lg:!gap-gap-
                >
                  <sofa-activity-card
                    v-for=" (activity, index) in resourceContents" :key="index" :activity="activity"
  ...
  <template v-else>
    <div class="w-full flex flex-col gap-3 md:!px-0 px-4">
      <sofa-empty-state :title="'No result found'" :subTitle="'We could not find any course that matches your search'"
        :actionLabel="'Clear search'" :action="
                  () => {
                    defaultValue = ' ';
                    search();
                  }
      ...
      <div class="w-full flex flex-col gap-3">
        ...
        <div
          ...
          <div class="mdlg:!w-full mdlg:!flex mdlg:!flex-col mdlg:!gap-4 flex flex-row gap-lg:!gap-gap-
                >
                  <sofa-activity-card
                    v-for=" (activity, index) in quizContents" :key="index" :activity="activity"
            :custom-class="'cursor-pointer'" @click="
                      Logic.Common.GoToRoute(
  ...
  <template v-else>
        <div class="w-full flex flex-col gap-3 md:!px-0 px-4">
          <sofa-empty-state :title="'No result found'" :subTitle="'We could not find any quiz that matches your search'"
            :actionLabel="'Clear search'" :action="() => {
                defaultValue = ' '
                search()
              }
  ...
  <div class="fixed bg-backgroundGray mdlg:!hidden bottom-0 left-0 px-4 py-4 flex flex-col w-full z-50">
    <div class="bg-primaryPurple custom-border py-3 flex flex-row items-center justify-center gap-
