<template>
	<expanded-layout layout-style="!justify-between bg-deepGray text-white" :hide="{ top: true, bottom: true }"
			...
			<template #default="{ game, extras: gameExtras, questions: gameQuestions, participants }">
				<QuizWrapper :id="game.quizId" :questions="gameQuestions">
					<template #default="{ quiz, questions, extras }">
						<Quiz v-model:answer="extras.answer" :index="extras.index" :title="quiz.title"
							:questions="questions" :show-counter="false" :option-state="extras.optionState" :is-dark="true"
							:right-button="{
	...
	click: () => Logic.Common.GoToRoute('/library'),
							...
							<template #header>
								<div class="px-4 pt-4 md:pt-8 w-full flex justify-center shadow-custom">
									<div class="lg:!w-[50%] mdlg:!w-[70%] md:!w-[80%] w-full flex gap-3 bg-white text-deepGray p-4 rounded-custom"
										...
										<template v-else>
											<SofaImageLoader :photo-url="quiz.photo?.link"
											...
											<div class="w-full flex flex-col h-full gap-2">
												...
												<div class="flex gap-2 items-center">
													...
													<SofaNormalText color="text-primaryPurple" :content="`${game.questions.length} question${game.questions.length > 1 ? 's' : ''
														}`" />
												</div>
							...
							<template #default>
								<div class="w-full h-full overflow-y-auto flex flex-col gap-4 my-auto py-4 items-center">
									...
									<SofaNormalText color="text-white" class="-mt-4" :content="gameExtras.isMine
