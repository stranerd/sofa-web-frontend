				...
				<QuizWrapper :id="game.quizId" :questions="questions">
					<template v-slot="{ quiz, questions, extras }">
						<Quiz v-model:index="extras.index" :title="quiz.title" :questions="questions"
							...
							<template v-slot>
								<div class="w-full flex flex-col h-full overflow-y-auto py-4 items-center">
									<SofaHeaderText class="md:!text-3xl text-xl" color="text-white" content="Lobby" />
									<SofaNormalText color="text-white" class="mb-4"
									...
									<div v-for="user in participants" :key="user.id"
										class="w-full flex flex-col items-center justify-center p-3 custom-border border-4 bg-white mb-4"
										:class="user.id === id ? 'border-hoverBlue' : 'border-transparent'">
										<SofaNormalText color="text-deepGray" class="!font-semibold"
											:content="user.id === id ? 'You' : user.bio.name.full" />
									</div>
									<div v-if="participants.length == 0"
										class="w-full flex flex-col items-center justify-center p-3 custom-border border-2 bg-white">
