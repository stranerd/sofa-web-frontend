<template>
  <sofa-form-wrapper :parentRefs="parentRefs" ref="formComp" class="w-full flex flex-col gap-4">
    <div class="w-full md:!grid md:!grid-cols-2 flex flex-col-reverse gap-4 md:!gap-0 md:!gap-4 gap-0">
      <div class="col-span-1 w-full flex flex-col gap-3">
        ...
        <sofa-textarea :hasTitle="false"
          ...
          :placeholder="'Description'" :richEditor="false" ref="description" v-model="quizSettingsForm.description" />
      ...
      <div class="w-full flex flex-row flex-wrap items-center">
        <template v-for="(item, index) in quizSettingsForm.tags" :key="index">
          <div class="py-2 pr-2" v-if="item != 'Not set'">
            <div class="py-2 px-3 border-[2px] flex flex-row items-center gap-stom-border border-[#E1E6EB]">
    ...
    <div
      ...
      <div class="mdlg:!w-auto w-full mdlg:!flex mdlg:!flex-row mdlg:!gap-id grid-cols-2 gap-2 mdlg:!gap-0 items-center">
        ...
        <div class="mdlg:!w-auto col-span-1 flex flex-col">
          <sofa-button :padding="'px-5 mdlg:!py-2 py-3'" :customClass="'mdlg:!w-auto w-full'"
            v-if="quiz && quiz.status != 'published'" @click.prevent="Logic.Study.PublishQuiz(quiz.id)">
            Publish
          </sofa-button>
        </div>
      </div>
