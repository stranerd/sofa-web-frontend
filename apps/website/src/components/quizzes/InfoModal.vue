<template>
  <div class="w-full flex flex-col gap-4">
    <div class="w-full flex flex-col gap-4" v-if="!isExplanation">
      <div class="bg-primaryPurple custom-border px-4 py-4 flex flex-col gap-2">
        <div class="flex flex-row items-center justify-start gap-2" v-for="(item, index) in allSteps" :key="index">
          <span class="h-[4px] w-[4px] rounded-full bg-white"> </span>
          <sofa-normal-text :color="'text-white'" :isHtml="true" :content="item">
          </sofa-normal-text>
        </div>
      </div>

      <div class="w-full flex flex-row gap-ems-center justify-center">
        <sofa-checkbox :customClass="'!w-auto'" v-model="dontShowAgain"><sofa-normal-text :color="'text-grayColor'">Don’t
            show again</sofa-normal-text></sofa-checkbox>
      </div>
    </div>
    ...
    <div
      ...
      <sofa-button :padding="'px-5 py-2'" :bgColor="'bg-white'" :textColor="'text-grayColor'"
        :customClass="'border-[1px] border-gray-100 hidden mdlg:!inline-block'" @click.prevent="close ? close() : null">
        Exit
      </sofa-button>
...
import { SofaButton, SofaCheckbox, SofaNormalText } from "sofa-ui-components"
