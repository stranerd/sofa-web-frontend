<template>
	<expanded-layout :hide="{ bottom: true }" width="mdlg:!w-[75%] lg:!w-[60%]" layoutStyle="mdlg:py-5">
		...
		<div
			:class="`${index ? 'grid' : 'hidden mdlg:grid'} flex-grow overflow-y-auto grid-cols-12 w-full gap-5 text-left`">
					...
					<div v-for="option in settingOptions" :key="option.title"
						...
						<template v-for="(optionItem, index) in option.subPages" :key="optionItem.title">
							<router-link class="w-full flex flex-col items-start p-2 hover:bg-skyBlue rounded-lg"
								:class="{ 'border-t border-lightGray mdlg:border-none': index !== 0 }"
