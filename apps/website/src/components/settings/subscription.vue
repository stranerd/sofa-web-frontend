	...
	<div class="w-full flex flex-col gap-5 mdlg:!px-0 px-4">
					...
					<div class="w-full flex flex-col gap-2 items-start">
						<sofa-header-text :color="'text-primaryPurple !text-2xl !font-extrabold'">
							{{ userWallet.subscription.current.id == "premium-plan"
								? "Stranerd Premium" : "Stranerd for organization" }}
						</sofa-header-text>

						<div class="w-full flex flex-col gap-1 pb-1">
							<sofa-normal-text>
								Expires {{ formatTime(userWallet.subscription.current.expiredAt) }}
							</sofa-normal-text>
							<sofa-normal-text v-if="userWallet.subscription.current.expiredAt > Date.now()">
								{{ Logic.Common.daysDiff(new Date(), new Date(userWallet.subscription.current.expiredAt)) }}
								days left
							</sofa-normal-text>
							<sofa-normal-text v-else> Expired </sofa-normal-text>
						</div>

						<div v-if="userWallet.subscription.current.expiredAt > Date.now()"
		...
		<template v-if="!userWallet.subscription.active">
			...
			<div class="w-full flex flex-col gap-4 bg-white rounded-[16px] md:!px-5 md:!py-5 px-4 py-4 shadow-custom"
				...
				<div class="w-full flex flex-col gap-3">
					<div :class="`w-full flex-col gap-1 pb-2 items-start ${index != subscriptionInfo.length - 1 ? 'border-b border-lightGray' : ''} `"
				...
				<div class="w-full flex flex-row">
					<div class="w-auto flex flex-row">
						<sofa-button @click="subscibeToPlan('premium-plan')" :padding="'px-7 py-2'"
							:customClass="'!w-auto'">
							Subscribe
						</sofa-button>
					</div>
