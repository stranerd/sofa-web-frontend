        ...
        <template v-if="currentStep == 0">
          <div class="w-full flex flex-col gap-3 px-4 md:px-0">
            ...
            <div
              ...
              <sofa-normal-text :color="selectedhelpOption == option.key
                ? 'text-primaryPurple'
                : 'text-[#78828C]'
                ">
                {{ option.title }}
        ...
        <template v-if="currentStep == 1">
          <div class="w-full flex flex-col gap-3 px-4 md:px-0">
            <sofa-textarea :padding="'px-3 py-4'" :custom-class="'bg-backgroundGray custom-border'"
              :placeholder="'Tell the tutor why you need him/her'" v-model="reasonForRequest" :rich-editor="false"
          ...
          <div class="w-full flex flex-col gap-3 md:!px-0 px-4 flex-grow">
            <template v-if="allTutors.length">
              <div :class="`w-full custom-border bg-[#F1F6FA] ${selectedTutorId == tutor.id
                ...
                : ''
                } px-4 py-4 flex flex-row items-center gap-rsor-pointer`" v-for="(tutor, index) in allTutors"
