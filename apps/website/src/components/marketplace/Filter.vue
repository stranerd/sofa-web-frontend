<template>
  <div class="w-full flex flex-col gap-6">
    <div class="w-full flex flex-col gap-3 px-4" v-if="selectedOptions.length">
      ...
      <div class="flex flex-row gap-ex-wrap items-center">
        <div v-for="(option, index) in selectedOptions" :key="index" class="w-auto pb-2">
          <span class="px-4 py-2 bg-primaryPurple custom-border flex flex-row items-center justify-center gap-
          >
            <sofa-normal-text :color="'text-white'">{{
              option.name
                ...
                <sofa-icon
                  ...
                  @click=" toggelOption(option)" :name="'close-white'" />
          </span>
        </div>
      </div>
    </div>
