<template>
  <div class="w-full flex flex-col space-y-6">
    ...
    >
      ...
      <div class="flex flex-row space-x-3 flex-wrap items-center">
        <div
        ...
        >
          ...
          >
            ...
            <sofa-icon
              :customClass="'h-[18px] cursor-pointer'"
              @click="toggelOption(option)"
              :name="'close-white'"
            />
          </span>
        </div>
    ...
    >
      ...
      <div class="w-full flex flex-row flex-wrap gap-3" v-if="option.opened">
        <span
          :class="`px-4 py-2  ${
            optionIsSelected(item.id) ? 'bg-primaryPurple' : 'bg-[#EFF2F5]'
          } custom-border flex flex-row items-center justify-center space-x-1  cursor-pointer`"
          v-for="(item, index) in option.options"
          :key="index"
          @click="toggelOption(item)"
        >
          <sofa-normal-text
            :color="`${
              optionIsSelected(item.id) ? 'text-white' : 'text-deepGray'
            } `"
  ...
  <div
    class="w-full flex flex-col bg-white mdlg:!hidden px-4 py-4 bottom-0 fixed left-0"
