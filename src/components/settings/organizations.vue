  ...
  <div class="w-full flex flex-col space-y-5 mdlg:!px-0 px-4">
    <div
      :class="`w-full flex flex-col space-y-4 mdlg:bg-white mdlg:rounded-[16px]  md:!px-5 md:!py-5 mdlg:px-4 mdlg:py-4 mdlg:shadow-custom ${
        allOrganizations.length > 0 ? 'bg-white rounded-[8px] px-3 py-3' : ''
      }`"
    >
      <div
        class="w-full flex-row items-center justify-between md:flex hidden"
        v-if="
      ...
      <sofa-empty-state
        ...
        :subTitle="'Your are not a student of any organization'"
      />
    </div>
