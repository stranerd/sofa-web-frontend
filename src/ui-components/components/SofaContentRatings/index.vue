<template>
	<div :class="`w-full flex flex-col ${hasWhiteBox ? 'gap-4' : 'gap-3'}  pb-4`">
			...
			<div class="flex flex-col py-4 px-2 mdlg:!w-[200px] w-[200px]">
				<div class="mdlg:!px-7 px-3 py-4 flex flex-col gap-2 items-center justify-center border-r-2 border-darkLightGray">
					<div class="flex flex-row">
						<SofaNormalText :custom-class="'mdlg:!text-xl !text-lg'">
							{{ data.avg }}
						</SofaNormalText>
						<SofaNormalText :custom-class="'mdlg:!text-xl  !text-lg'" :color="'text-grayColor'"> /5 </SofaNormalText>
					</div>
		...
		<div
			...
			<div>
				<SofaAvatar :photo-url="review.user.photoUrl" :size="'44'" :user-id="review.user.id" />
			</div>

			<div class="flex flex-col gap-1">
				<SofaNormalText :custom-class="'!font-semibold'">{{ review.user.name }}</SofaNormalText>
