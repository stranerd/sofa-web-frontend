<template>
	<div
			...
			<div class="hidden mdlg:!flex lg:!flex flex-row items-center gap-4">
				...
				<div class="flex flex-col relative" tabindex="10" @blur="showNotification = false">
					<div
						...
						<SofaIcon :custom-class="'h-[16px]'" :name="'bell'" />
					</div>
					<div
				...
				<SofaAvatar :size="'36'" :photo-url="user?.bio?.photo?.link" @click="Logic.Common.GoToRoute('/settings/profile')" />
			</div>
		</template>

		<template v-if="type == 'subpage'">
			<div class="flex flex-row gap-4 items-center">
				<SofaIcon :custom-class="'h-[12px] cursor-pointer'" :name="'back-arrow'" @click="Logic.Common.goBack()" />
				<SofaHeaderText :custom-class="'!font-bold py-4'" :content="title" />
					...
					<template v-if="action.isIcon && !action.hide">
						<div class="flex flex-row gap-4 border-r border-darkLightGray items-center pr-3 cursor-pointer">
							<div
								...
								@click="icon.handler()">
								<SofaIcon :name="icon.icon" :custom-class="icon.size" />
		...
		<!-- Notification modal -->
		<SofaModalOld v-if="showNotification" :close="() => (showNotification = false)" :custom-class="'mdlg:!hidden'">
