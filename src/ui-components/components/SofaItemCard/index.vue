<template>
	<component :is="as" :class="`col-span-1 flex flex-col gap-2 px-3 py-3 cursor-pointer rounded-custom ${customClass}`">
		<SofaImageLoader custom-class="w-full mdlg:!h-[155px] h-[120px] rounded-custom relative" :photo-url="content.image">
			<div
		...
		<SofaNormalText :custom-class="'!font-bold w-full flex pt-1 justify-start text-left !line-clamp-1'">
			{{ content.title }}
		</SofaNormalText>
		...
		<div v-if="content.user" class="flex gap-2 items-center justify-between pt-1">
			<a class="gap-2 flex items-center" @click.stop.prevent="Logic.Common.GoToRoute(`/profile/${content.user.id}`)">
				<SofaAvatar :size="'20'" :photo-url="content.user.bio.photo?.link" :user-id="content.user.id" />
				...
				<SofaIcon v-if="content.user.roles.isVerified" :name="'verify'" :custom-class="'h-[13px]'" />
				<SofaIcon v-if="content.user.type?.type === 'teacher'" :name="'tutor-bagde'" :custom-class="'h-[13px]'" />
			</a>

			<SofaIcon :name="'bookmark'" :custom-class="'h-[18px] '" @click.stop="bookmarkAction ? bookmarkAction() : null" />
		</div>
	</component>
</template>
