	...
	<div
		...
		<template v-for="message in messages" :key="message.hash">
			<div
				...
				<SofaAvatar :photo-url="users[message.userId]?.photoUrl" class="flex-shrink-0" :size="'27'" />
				<div class="p-3 rounded-custom text-left bg-lightBlue flex flex-col gap-1 justify-start">
					<SofaNormalText :custom-class="'!font-semibold'" :color="'text-[#3296C8]'">
						{{ users[message.userId]?.name }}
					</SofaNormalText>
					<SofaNormalText :custom-class="'text-left'" :is-html="true" :content="message.body"></SofaNormalText>
				</div>
			</div>
