<template>
	<div class="p-4 mdlg:p-6 flex flex-col gap-4 h-full">
		<div class="flex w-full items-center gap-2 justify-between">
			...
			<SofaHeading size="title" content="Create a subject" />
			<SofaIcon v-if="!$screen.desktop" class="h-[16px]" name="circle-close" @click="close" />
...
const isDisabled = computed(() => {
	if (stage.value === 1) return !factory.isValid('title')
	if (stage.value === 2) return !factory.valid
	return true
})
