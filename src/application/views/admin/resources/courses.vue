	...
	<AdminLayout title="Courses">
		<div class="flex flex-col bg-white rounded-2xl">
			<div class="flex justify-between items-center py-1">
				<SofaHeading content="Courses" class="px-4 w-[60%]" />
					...
					<template #data-action="{ data }">
						<div class="flex items-center justify-between gap-6">
							<SofaButton
								...
								padding="py-1"
								@click="useEditCourse(data.id).deleteCourse()">
								Delete
							</SofaButton>
...
import { useCoursesList, useEditCourse } from '@app/composables/study/courses'
import { CourseEntity } from '@modules/study'
const { currentlyViewing, currentViewingIndex, limit, total, next, previous, canNext, canPrev } = useCoursesList()
const handleClick = (data: CourseEntity) => {
	useModals().study.editCourse.open({
		course: data,
	})
}
</script>
